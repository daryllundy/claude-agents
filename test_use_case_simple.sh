...
if declare -F format_use_case >/dev/null 2>&1 || grep -q "^format_use_case()" scripts/recommend_agents.sh; then
  test_pass
else
...
if declare -F get_terminal_width >/dev/null 2>&1 || grep -q "^get_terminal_width()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if declare -F format_use_case_auto >/dev/null 2>&1 || grep -q "^format_use_case_auto()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if declare -F get_use_case_safe >/dev/null 2>&1 || grep -q "^get_use_case_safe()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if declare -F format_use_case_cached >/dev/null 2>&1 || grep -q "^format_use_case_cached()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if grep -q "^declare -A USE_CASE_WRAP_CACHE" scripts/recommend_agents.sh; then
  test_pass
else
...
if grep -q "use_case.*get_use_case_safe" scripts/recommend_agents.sh; then
  test_pass
else
...
if grep -q 'Use for:' scripts/recommend_agents.sh; then
  test_pass
else
...
if grep -A 10 "^render_agent_item()" scripts/recommend_agents.sh | grep -q 'use_case.*6'; then
  test_pass
else
...
if grep -A 60 "^export_profile()" scripts/recommend_agents.sh | grep -q 'use_case'; then
  test_pass
else
...
if grep -A 50 "^export_profile()" scripts/recommend_agents.sh | grep -q 'use_case_escaped'; then
  test_pass
else
...
if declare -F import_profile >/dev/null 2>&1 || grep -q "^import_profile()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if grep -A 20 "^render_agent_list()" scripts/recommend_agents.sh | grep -q 'use_cases_ref'; then
  test_pass
else
...
if grep -A 30 "^render_agent_item()" scripts/recommend_agents.sh | grep -q 'is_current.*1'; then
  test_pass
else
...
if grep -A 50 "^render_agent_list()" scripts/recommend_agents.sh | grep -q 'use_cases_ref\['; then
  test_pass
else
...
if declare -F validate_use_cases >/dev/null 2>&1 || grep -q "^validate_use_cases()" scripts/recommend_agents.sh; then
  test_pass
else
  test_fail "Function not found"
fi
...
if grep -A 20 "^validate_use_cases()" scripts/recommend_agents.sh | grep -q 'AGENT_USE_CASES'; then
  test_pass
else
...
if ./scripts/recommend_agents.sh --dry-run >/dev/null 2>&1; then
  test_pass
else
...
if ./scripts/recommend_agents.sh --dry-run 2>&1 | grep -q "Use for:"; then
  test_pass
else
...
if ./scripts/recommend_agents.sh --dry-run 2>&1 | head -20 | grep -q "Loaded.*agent.*patterns"; then
  test_pass
else
